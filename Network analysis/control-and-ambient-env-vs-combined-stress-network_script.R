...
module load R/4.1.0
R CMD BATCH inference.R3

#The inference.R3 file contains the following R scripts.
library(devtools)
library(SpiecEasi)
library(microbiome)
library(phyloseq)
...
net.c<- subset_samples(net.c1, environment !="GH" & IET !="COE" & IET !="COM" & IET !="IAM" & IET !="IAE")
...
pdf("./combined.pdf", width = 50, height = 25)
p <- plot(netprops_env,
          sameLayout = TRUE,
          layoutGroup = 1,
          repulsion = 1,
