...
module load R/4.1.0
R CMD BATCH inference.R

#The inference.R file contains the exact script for network analysis which is as follows.
...
net.c<- subset_samples(net.c1, environment !="GH" & innoculation !="Innoculated")
...
environment <- env_pheno$environment
...
summary(netprops_env, groupNames = c("Ambient", "Ozone"))



pdf("/scratch/aubrrb/netcomi/abioticstressonly.pdf", width = 50, height = 25)
p <- plot(netprops_env,
          sameLayout = TRUE,
          layoutGroup = 1,
          repulsion = 1,
          groupNames = c("Ambient","Ozone"),
...
summary(net_env_comp, pAdjust = TRUE,
        groupNames = c("Ambient", "Ozone"), digitsPval = 6)
